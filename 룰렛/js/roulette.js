...
$(document).on("keydown", function (evt) {
  ...
  if (evt.key === "Enter") {
    ...
    if ($target.is(".item-title")) {
      $target.blur().closest("li").next("li").find(".item-title").focus();
    }
...
function Roulette() {
  ...
  function OnUpdate(timestamp) {
    ...
    if (R.angularDamping) {
      if (R.selectAngle == 0) {
        R.angularVelocity -= Math.max(20, Math.min(R.angularVelocity * 0.5, R.angularDamping)) * (33 / 1000);
      } else if (R.selectAngle > 0 && R.angularVelocity > 2) {
        R.angularVelocity -= Math.max(20, Math.min(R.angularVelocity * 0.5, R.angularDamping)) * (33 / 1000);
      }
    }
    // if (R.angularVelocity > 0 && R.angularVelocity < R.needStopAngularVelocity) {
    //   console.log("S,R,C,V", 360 - R.selectAngle, R.rotation, 360 - R.selectAngle - R.rotation);
    // }
    //####################
    ...
    if (R.angularVelocity) {
      var cap = R.angularVelocity * (33 / 1000); //(R.deltaTime / 1000);

      // 근처왔을때
      if (R.angularVelocity < R.needStopAngularVelocity) {
        let s_cap_r = 360 - R.selectAngle - R.rotation;
        if (s_cap_r < 0) s_cap_r = 360 - s_cap_r;
        //console.log(R.angularVelocity, cap, s_cap_r);
  ...
  R.Launch = function () {
    ...
    //setup value
    R.needStopAngularVelocity = 10;
    // ===
    //localStorage.selectIndex = -1;
  };
  R.Stop = function () {
    R.stopping = Date.now();
    // R.angularDamping = 360 * (0.5 + 0.1 * Math.random());

    //=========================
    let index = localStorage.selectIndex;
    if (index > -1) {
      R.startRotation = R.rotation;

      let angle = R.Items[index].angle;
      let angleSum = R.Items[index].angleSum;

      let min = angleSum - angle;
      let max = angleSum;

      // 원하는 범위내 랜덤.
      R.selectAngle = (max - min) * Math.random() + min;

      // 더할 각도
      let addAngle = 360 - R.selectAngle - R.rotation;
      ...
      R.angularDamping = startDp - addAngle * capDp - fixDp;

      // console.log("#Check");
      // console.log("index", index);
      // console.log("R.selectAngle", R.selectAngle);
      // console.log("R.prevRotation", R.prevRotation);
      // console.log("R.rotation", R.rotation);
      // console.log("R.currotation", R.rotation);
      // console.log("addAngle", addAngle);
      // console.log("R.angularDamping", R.angularDamping);
    } else {
      R.angularDamping = 360 * (0.5 + 0.1 * Math.random());
    }
    //----------------------
...
$(document).ready(function () {
  ...
  R.onStop = function () {
    //console.log("Stop:: S,R,C", 360 - R.selectAngle, R.rotation, 360 - R.selectAngle - R.rotation);

    $result
      .removeClass("out")
      .text(R.current || "돌려돌려 돌림판!")
      .show()
      .addClass("play");
    clearTimeout(it);
...
(function () {
  ...
  $(document)
    .on("mouseenter mousemove", ".item-weight-sub, .item-weight-add", function (e) {
      move();
      show();
    })
...
function monitor() {
  // setInterval(() => {
  //   console.log("R.angularVelocity", R.angularVelocity);
  // }, 1000 * 1);
}
