...
def init_db():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS players (name TEXT PRIMARY KEY, score INTEGER, contribution INTEGER)")
...
def load_data():
    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute("SELECT name, score, contribution FROM players ORDER BY contribution DESC")
        ...
        try:
            with sqlite3.connect(DB_FILE) as conn:
                for new_p in new_data.get("bjs", []):
                    ...
                    if diff != 0:
                        conn.execute(
                            "INSERT INTO donation_history (timestamp, name, amount, current_total, message, source) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            for bj in new_data.get("bjs", []):
                cursor.execute("INSERT INTO players (name, score, contribution) VALUES (?, ?, ?)", (bj["name"], bj["score"], bj.get("contribution", 0)))
            
            for key, value in new_data.items():
                if key != "bjs":
                    cursor.execute("INSERT OR REPLACE INTO kv_store (key, value) VALUES (?, ?)", (key, json.dumps(value, ensure_ascii=False)))
            conn.commit()
    except Exception as e:
...
def get_snapshots():
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, timestamp, summary FROM snapshots ORDER BY id DESC")
...
def create_manual_snapshot():
    try:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            conn.execute(
                "INSERT INTO snapshots (timestamp, state_json, summary) VALUES (?, ?, ?)",
...
def restore_snapshot():
    try:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT state_json FROM snapshots WHERE id = ?", (snap_id,))
            row = cursor.fetchone()
            if not row:
...
def restore_by_time():
    try:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute("SELECT key, value FROM kv_store WHERE key = 'target_goal'")
...
if __name__ == '__main__':
    if run_login_gui():
        ...
        lbl_status = tk.Label(root, text='🟢 실시간 방송 정산 엔진 구동 중 (Port: 5000)', fg='#ffffff', bg='#111113', font=('Malgun Gothic', 11, 'bold'))
        ...
        lbl_otp_info = tk.Label(root, text='* 최초 등록 방법: 스마트폰 구글 OTP 앱에서 위 키를 입력하거나,\n서버 PC 브라우저로 http://localhost:5000/setup 에 접속해 QR 코드를 스캔하세요.', fg='#8e8e93', bg='#111113', font=('Malgun Gothic', 8), justify='center')
        lbl_otp_info.pack(pady=5)
        
        frame_btns = tk.Frame(root, bg='#111113')
        frame_btns.pack(pady=20)
        
        btn_ctrl = tk.Button(frame_btns, text='💻 제어 센터 (조종실)', command=lambda: open_link('http://localhost:5000/controller'), fg='#000000', bg='#00ffcc', activebackground='#00cca3', font=('Malgun Gothic', 10, 'bold'), width=18, height=2, relief='flat')
        btn_ctrl.pack(side=tk.LEFT, padx=10)
        
        btn_ovr = tk.Button(frame_btns, text='🎬 송출용 오버레이', command=lambda: open_link('http://localhost:5000/overlay'), fg='#ffffff', bg='#333336', activebackground='#444448', font=('Malgun Gothic', 10, 'bold'), width=18, height=2, relief='flat')
